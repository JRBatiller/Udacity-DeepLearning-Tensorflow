...
def maybe_download(filename, expected_bytes, force=False, data_root='.'):
...
def reformat_cube(dataset, labels, image_size=28, num_labels=10, num_channels=1):
...
def maybe_pickle_unique(data_folders, min_num_images_per_class, force=False):
    dataset_names = []
    for folder in data_folders:
        unique_data_list = []
        set_filename = folder + '_unique.pickle'
        dataset_names.append(set_filename)
        if os.path.exists(set_filename) and not force:
            # You may override by setting force=True.
            print('%s already present - Skipping pickling.' % set_filename)
            ...
            print('Pickling %s.' % set_filename)
            dataset = load_letter(folder, min_num_images_per_class)
            new_data = [x for x in unique_data_list if not x in dataset]
            try:
                with open(set_filename, 'wb') as f:
